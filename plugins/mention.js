...
*/
const {SESSION,LOGOSK,AUDIO_DATA,SUDO} = require('../config');
...
addCommand({pattern: 'mention ?(.*)', fromMe: true,dontAddCommandList: true}, (async (message, match) => {return;}));
addCommand({on: 'text' ,fromMe: false}, (async (message, match) => {
...
if (audio) {
try { var res = await addInfo('mention_msg.mp3',tit,art,'Raganork audio metadata',await skbuffer(LOGOSK)) } catch { return await message.sendMessage('Error on parsing audio'); }
return await message.client.sendMessage(message.jid, res, MessageType.audio, { mimetype: Mimetype.mp4Audio,duration: duration, quoted : message.data, ptt: true})}
})}
}));
